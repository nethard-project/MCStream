use crate::{
    ...
    error::McStreamError,
    palette,
    compression::{compress_data, decompress_data, compression_type_from_u8},
    CompressionType
};
...
pub fn serialize_chunk(chunk: &ChunkData) -> Result<Vec<u8>, McStreamError> {
    ...
    let nbt_blocks: Vec<&Block> = chunk.blocks.iter()
...
pub fn deserialize_chunk(data: &[u8], pos: ChunkPos) -> Result<ChunkData, McStreamError> {
    ...
    if nbt_count as usize != nbt_blocks.len() {
        return Err(McStreamError::NbtError("NBT数据数量与标记不一致".to_string()));
    }
