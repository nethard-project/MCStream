use mcstream::{McsEncoder, McsDecoder, CompressionType, McStreamError};
use std::path::Path;
...
fn main() -> Result<(), McStreamError> {
    ...
    println!("文件信息:");
    println!("  格式版本: {}.{}", (header.version >> 8) & 0xFF, header.version & 0xFF);
    ...
    for (pos, chunk) in decoder.get_chunks() {
        ...
        for block in &chunk.blocks {
            ...
            print!("  方块 {} 位于 [{}, {}, {}]", block_id, global_x, global_y, global_z);
